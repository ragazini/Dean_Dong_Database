...
def main():
    while True:
        ...
        # call login function and pass "username" as its argument
            login(username)
...
def display_db():
    ...
    if pyfiglet_installed:
        ...
        stylized_text = figlet.renderText(
            f"Hasta la vista, {uppercase_username}")
        print(stylized_text)
    else:
        print(
...
def register():
    while True:
        new_username = input(
            "Choose your username (from 6 to 12 characters long): ")
        ...
        if 6 <= len(new_username) <= 12 and new_username not in keyList:
            ...
            if set_password_option == 'y':
                ...
                while True:
                    try:
                        option = int(
                            input("Enter an option for password strength (1/2/3): "))
